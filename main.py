...
async def kie_callback(payload: dict):
    ...
    if state == "success" or state == "SUCCESS":
        ...
        # ===== RÉCUPÉRATION DE L'URL VIDÉO (Sora 2 OU Veo 3.1) =====
        video_url = None
        ...
        if result_json_str:
            try:
                ...
                print(f"📹 Sora 2 video URL: {video_url}")
            except Exception as e:
                print(f"⚠️ Error parsing Sora resultJson: {e}")
        
        # FORMAT VEO 3.1 : videoUrl directement dans data
        if not video_url:
            video_url = data.get('videoUrl') or data.get('video_url')
            if video_url:
                print(f"📹 Veo 3.1 video URL: {video_url}")
        
        # Vérifier qu'on a bien une URL
        if not video_url:
            print("⚠️ No video URL in result (neither Sora nor Veo format)")
        ...
        if task_id:
            try:
                ...
                # ===== MULTI-CLIP =====
                    ...
                    # ===== TOUS LES CLIPS PRÊTS → CONCATÉNATION =====
